...
class GeneralMidiParser(SingletrackMidiParser):
    ...
    def merge_tracks_to_singletrack(mid):
        if mid.type != 1:
            # If MIDI file is not type 1 (multiple tracks), skip merging
            return
        ...
        combined_messages.append(mido.MetaMessage('end_of_track', time=0))
        mid.tracks = [mido.MidiTrack(combined_messages)]
        mid.type = 0  # Change to type 0 (single track)

    @classmethod
    def load_midi(cls, midi_path):
        mid = mido.MidiFile(midi_path)
        try:
            cls.merge_tracks_to_singletrack(mid)
        except Exception as e:
            print(f"Error processing MIDI file {midi_path}: {e}")
            return None
        assert mid.type == cls.SINGLETRACK_MIDI_CODE, "Failed to convert to single-track MIDI."
        return mid
